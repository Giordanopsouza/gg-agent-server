...
def test_dockerfile_matches_standard_modal_image_evidence() -> None:
    source = (
        Path(__file__).resolve().parents[2] / "sandboxes" / "Dockerfile"
    ).read_text()
...
def _fake_repo(tmp_path: Path) -> Path:
    repo = tmp_path / "repo"
    (repo / "sandboxes").mkdir(parents=True)
    (repo / "sandboxes" / "Dockerfile").write_text("FROM scratch\n")
    return repo
...
def test_repository_root_is_cwd_even_when_module_is_installed(
    ...
    monkeypatch.chdir(repo / "sandboxes")
