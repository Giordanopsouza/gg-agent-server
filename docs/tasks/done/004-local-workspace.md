...
- [x] `write_file(relative_or_absolute, content)` creates parent dirs and writes bytes.
- [x] `read_file` returns the bytes just written.
- [x] `execute_command("pwd")` with no cwd runs inside `working_dir`.
- [x] Tests use a temp directory and assert the file exists on disk.
...
### [SWE] 2026-08-21 16:37 — Local workspace landed

`gg.sdk.local_workspace` with `LocalWorkspace`, `CommandResult`, and `test_local_workspace.py`. write/read/execute_command with cwd defaulting to `working_dir`.
