...
### [SWE] 2026-09-25 — Integração com a fundação 077

Branch 061 atualizada com a `main`. Migração `active_web_sessions` aplicada primeiro ao Supabase Docker local e reconstruída do zero; smoke local criou um perfil UUID pelo papel `gg_runtime` e confirmou que o logout remove a sessão ativa. Os 24 testes pgTAP passaram, e o advisor local não encontrou problemas. A mesma migração foi aplicada ao projeto publicado com versão `20260926010913`; consulta somente leitura confirmou execução apenas pelo papel `gg_runtime`. O runtime agora exige o banco privado para login web e rejeita sessões revogadas em `/auth/session`. Falta repetir o fluxo Google real com esta nova versão do runtime e validar na URL HTTPS publicada.
